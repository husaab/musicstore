...
public class Library
{
	...
	String errorMsg = ""; //#removing errormsg variable as we will replace it now
	...
	public void download(AudioContent content)
	{
		...
		if (content.getType().equals(Podcast.TYPENAME))
		{
			...
			{
				System.out.println("Podcast already downloaded");
			}
			else{
  ...
  //Sort songs in library by year
	...
	public void playPodcast(int index, int season, int episode)
	{
		...
		{
			throw new PodcastNotFoundException("Season not found");
		}
		
		if (episode < 1 || episode > podcast.getSeasons().get(season-1).episodeTitles.size())
		{
			throw new PodcastNotFoundException("Episode not found");
		}
	...
	public void printPodcastEpisodes(int index, int season)
	{
		...
		Podcast podcast = podcasts.get(index-1);
		if(index < 1 || index > podcast.getSeasons().size()){
			throw new PodcastNotFoundException("Season not found");
		}
	...
	public void playAudioBook(int index, int chapter)
	{
		...
		{
			throw new AudioBookNotFoundException("AudioBook Chapter Not Found");
		}
   ...
   */
	...
	public void makePlaylist(String title)
	{
		...
		{
			throw new PlaylistExistenceException("Playlist " + title + " Already Exists"); 
		}
		playlists.add(pl);
	}
	...
	public void addContentToPlaylist(String type, int index, String playlistTitle)
	{
		...
		int playlistindex = playlists.indexOf(new Playlist(playlistTitle));

		if(playlistindex == -1){	//if it is equal to -1 then throw exception, as indexOf gives -1 if not found
			throw new PlaylistNotFoundException("Playlist not found");
		}

		if(playlistindex != -1){	//if playlist index is not equal to -1, then run this
			...
			if (type.equalsIgnoreCase("SONG"))
			{
				...
				playlists.get(playlistindex).addContent(ac);
			}
...
class PodcastNotFoundException extends RuntimeException{
...
class PlaylistExistenceException extends RuntimeException{
	public PlaylistExistenceException(){}

	public PlaylistExistenceException(String message){
		super(message);
	}
}
